...
function App() {
    ...
    const [newName, setNewName] = useState([""]); //입력받는
    const [validation, setValidation] = useState(false); //검사
    ...
    const handleOnClick = (event) => {
        const newid = event.target.newid.value;
        const newpasswd = event.target.newpasswd.value;
    }
        /*
        const exist = userData.find(isExist);
        console.log(exist);
    ...
    */
    ...
    return ( 
        ...
        <form onSubmit={handleOnClick}>
            <input type="text" name="newid" placeholder="아이디" />
            <input type="text" name="newpasswd" placeholder="암호" />
            <button type="submit"> 검색 </button>
        </form>

        {validation && (
        <section className="validation">이미 존재함</section>
        )}
